                    ...
                    <div class="posts-area">
                        ...
                        @foreach ($blogs as $item)
                            <div class="post-auhtor-area {{ $loop->iteration == 2 ? 'box2' : '' }}">
                                ...
                                <div class="content">
                                    <a href="{{ route('blog.show', $item->slug) }}" class="date">
                                        <img src="{{ asset('frontend/assets/img/icons/calender1.svg') }}" alt="">
                                        {{ $item->created_at->format('d F Y') }}
                                    </a>

                                    <a href="{{ route('blog.show', $item->slug) }}" class="head">
                        ...
                        <div class="content-area">
                            <div class="tags-area">
                                <ul>
                                    ...
                                    <li><i class="fa-solid fa-calendar"></i> {{ $blog->created_at->format('d M Y') }}</li>
                                </ul>
                            </div>
                            <a href="{{ route('frontend.singleservice', $blog->slug) }}">{{ $blog->title }}</a>
                            <p>{{ Str::limit(strip_tags($blog->description), 90) }}</p>
                            <a href="{{ route('frontend.singleservice', $blog->slug) }}" class="readmore">Read More <i class="fa-solid fa-arrow-right"></i></a>
                        </div>
                    </div>
